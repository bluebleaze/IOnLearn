...
export const SimulateTaskModal: React.FC<SimulateTaskModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-xl p-0 overflow-hidden dark:bg-[#0B0F17] dark:border-slate-800">
        <DialogHeader className="p-5 border-b border-slate-200 dark:border-amber-950/40 bg-amber-50/70 dark:bg-amber-950/20 pr-12">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-amber-600 text-white flex items-center justify-center shadow-xs shrink-0">
              <FolderSync className="w-5 h-5" />
            </div>
            <div>
              <DialogTitle className="text-base font-bold text-slate-900 dark:text-amber-200">
                Simulasi Notifikasi Tugas Guru
              </DialogTitle>
              <DialogDescription className="text-xs text-amber-900/80 dark:text-amber-300/80 mt-0.5">
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto dark:bg-[#0B0F17]">
          ...
          <div className="space-y-3">
            {PRESET_CLASSROOM_ASSIGNMENTS.map((preset, idx) => (
              <div
                key={idx}
                className="p-4 rounded-2xl border border-slate-200/90 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-500/50 hover:bg-indigo-50/20 dark:hover:bg-indigo-950/30 bg-slate-50/50 dark:bg-slate-900/60 transition flex flex-col justify-between gap-3 group"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className="px-2.5 py-0.5 rounded-md text-[11px] font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900/40">
                      {preset.courseName}
                    </span>
