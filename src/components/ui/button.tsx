...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm font-semibold transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/20 disabled:pointer-events-none disabled:opacity-50 cursor-pointer active:scale-95 shrink-0 select-none",
  {
    variants: {
      variant: {
        default:
          "bg-indigo-600 text-white shadow-xs shadow-indigo-200 hover:bg-indigo-700",
        destructive:
        ...
        emerald:
          "bg-emerald-600 text-white shadow-xs shadow-emerald-200 hover:bg-emerald-700",
      },
      size: {
        default: "h-9 px-4 py-2 text-xs sm:text-sm",
        sm: "h-8 rounded-lg px-3 text-xs",
        lg: "h-11 rounded-2xl px-6 text-sm sm:text-base font-bold",
        icon: "h-9 w-9 rounded-xl",
        iconSm: "h-8 w-8 rounded-lg p-1.5",
        iconXs: "h-7 w-7 rounded-md p-1",
      },
    },
    defaultVariants: {
      variant: "default",
