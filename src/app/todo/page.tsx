  ...
  return (
    <Shell>
      <div className="space-y-6 max-w-4xl mx-auto pb-16">
        ...
        {showAddForm && (
          <form
            onSubmit={handleAddQuickTodo}
            className="bg-white dark:bg-[#171717] rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-[#222] space-y-3.5"
          >
            <div className="font-semibold text-xs text-slate-800 dark:text-[#e5e5e5]">
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 text-xs rounded-xl bg-slate-50 dark:bg-[#1f1f1f] border-0 text-slate-900 dark:text-[#f3f3f3] placeholder:text-slate-400 dark:placeholder:text-[#666] focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              autoFocus
            />

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1">
              <div>
                <label className="text-xs font-medium text-slate-500 dark:text-[#777] block mb-1">
                  Prioritas
                </label>
                <select
                  value={newPriority}
                  onChange={(e) => setNewPriority(e.target.value as any)}
                  className="w-full px-3 py-1.5 text-xs rounded-lg bg-slate-50 dark:bg-[#1f1f1f] border-0 text-slate-800 dark:text-[#e5e5e5] focus:outline-none"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-xs rounded-lg bg-slate-50 dark:bg-[#1f1f1f] border-0 text-slate-800 dark:text-[#e5e5e5] focus:outline-none placeholder:text-slate-400 dark:placeholder:text-[#666]"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              <Button
                ...
                className="text-xs text-slate-600 dark:text-[#888]"
              >
                Batal
              </Button>
              <Button
                type="submit"
                size="sm"
                className="text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg"
              >
                Simpan To-Do
              </Button>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-[#191919] p-1 rounded-xl text-xs overflow-x-auto no-scrollbar">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition-all cursor-pointer whitespace-nowrap ${
                filterTab === "all"
                  ...
                  : "text-slate-600 dark:text-[#8e8e8e]"
              }`}
            >
              Semua ({totalCount})
            </button>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition-all cursor-pointer whitespace-nowrap ${
                filterTab === "completed"
                  ? "bg-white dark:bg-[#252525] text-emerald-600 dark:text-emerald-400 shadow-sm font-semibold"
                  : "text-slate-600 dark:text-[#8e8e8e]"
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition-all cursor-pointer whitespace-nowrap inline-flex items-center gap-1 ${
                filterTab === "high"
                  ? "bg-white dark:bg-[#252525] text-rose-600 dark:text-rose-400 shadow-sm font-semibold"
                  : "text-slate-600 dark:text-[#8e8e8e]"
              }`}
            >
          ...
          <div className="w-full sm:w-64">
            <input
              ...
              className="w-full px-3 py-1.5 text-xs rounded-xl bg-slate-100 dark:bg-[#191919] border-0 text-slate-900 dark:text-[#f3f3f3] placeholder:text-slate-400 dark:placeholder:text-[#666] focus:outline-none"
            />
          </div>
        </div>
        ...
        {filteredTodos.length === 0 ? (
          filterTab === "active" && totalCount > 0 && activeCount === 0 ? (
            ...
            <div className="text-center py-16 px-6 bg-white dark:bg-[#161616] rounded-2xl border border-slate-200/80 dark:border-[#262626] shadow-2xs space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
                <Button
                  ...
                  size="sm"
                  onClick={() => setFilterTab("completed")}
                  className="text-xs rounded-xl border-slate-200 dark:border-[#2b2b2b]"
                >
                  Lihat To-Do yang Telah Selesai
                </Button>
                <Button
                  size="sm"
                  onClick={() => setShowAddForm(true)}
                  className="text-xs rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-[#f0f0f0] dark:hover:bg-white text-white dark:text-slate-900 shadow-2xs"
                >
            ...
            <div className="text-center py-16 px-4 bg-white dark:bg-[#161616] rounded-2xl border border-slate-200/80 dark:border-[#262626] shadow-2xs space-y-3">
              <div className="w-14 h-14 mx-auto rounded-2xl bg-slate-100 dark:bg-[#202020] flex items-center justify-center text-slate-500 dark:text-[#888]">
                  ...
                  <div className="flex items-start gap-3">
                              ...
                              {item.subtasks?.map((subtask) => (
                                ...
                                >
                                  <button
                                    ...
                                    className={`mt-0.5 w-4 h-4 rounded flex items-center justify-center transition-all cursor-pointer shrink-0 ${
                                      subtask.isCompleted
                                        ? "bg-emerald-500 text-white"
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 dark:text-[#737373] dark:hover:text-rose-400 hover:bg-slate-100 dark:hover:bg-[#202020] transition-colors cursor-pointer"
                        title="Hapus To-Do"
                      >
        ...
        {showClassroomModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white dark:bg-[#171717] rounded-2xl max-w-lg w-full p-5 space-y-4 shadow-xl border border-slate-100 dark:border-[#262626]">
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-[#222]">
                ...
                <Button
                  ...
                  className="text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl gap-2"
                >
