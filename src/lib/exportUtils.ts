...
/**
 * Direct download helper for CreatedDocument
 */
export async function downloadCreatedDocument(doc: CreatedDocument) {
  const ext = doc.type === "pdf" ? ".pdf" : ".docx";
  const defaultName = (doc.title.toLowerCase().replace(/[^a-z0-9]+/g, "_") || "dokumen") + ext;
